...
public class CheckIgpDownInterfaces implements Runnable
{
	...
	public CheckIgpDownInterfaces(String threadname, InventoryDatabase inventoryDatabase, Neo4jDatabase neo4jDatabase) {
		threadName = threadname;
		this.neo4jDatabase = neo4jDatabase;
		this.inventoryDatabase = inventoryDatabase;
		this.IgpNodes = new Hashtable<String, IgpNode>();
		logger.debug("created down interface checker Thread, threadname " + threadName);
	}
	...
	public void run()
	{
		...
		try {
			while (!stop) {
				...
				logger.info("checing down interfaces. if any of them is up will set its status to warn(4))");
				this.IgpNodes = neo4jDatabase.getIgpInterfacesWithStatus(16);
				if (this.IgpNodes != null) {
					...
					neo4jDatabase.setIgpInterfacesStatus(downIgpInterfaces,4,"igpDOWNinterfaceChecker");
				} else {
					logger.error("no down inteface found in the graph database");
					waitTime=120000;
				}				
	...
	private Hashtable<String, IgpNode> CollectIgpInterfaceAttributes(Hashtable<String, IgpNode> IgpNodes) 
	{
		...
		try {
			...
			snmp.listen();
			logger.debug("snmp inteface creadted with engine ID " + contextEngineId.toString());
		} catch (IOException e) {
		...
		while (iterators.hasNext()) {
			...
			if (engineIdBytes != null) {
				...
				logger.error("can not set snmp engineid" + (this.IgpNodes.get(key)).getIgpRouterID() + " ,removing from the list");
				iterators.remove();
			}
		}            
		...
		while (iterators.hasNext()) {
			...
			if (this.IgpNodes.get(key).getIgpinterfaces() == null) {
				logger.error("null interface list for node "+ this.IgpNodes.get(key).getIgpRouterID() + " , there may be error in the graph database or in the progmam. Graphdb should not return node without down interface");
				continue;
			}
			...
			while (iterators2.hasNext()) {
				...
				if(this.IgpNodes.get(key).getIgpinterfaces().get(igpint).getIfIndex() == null ) {
					logger.error("null ifIndex " + this.IgpNodes.get(key).getIgpRouterID()+ " interface number " + igpint );
					continue;
				}
				...
				logger.debug("sent snmp to node " + this.IgpNodes.get(key).getSnmpTarget().getAddress());
